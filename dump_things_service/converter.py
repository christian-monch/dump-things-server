...
def _do_convert_format(
    ...
    if input_format == 'XXX':  # Format.json:
    ...
    elif input_format == Format.json:  # 'XXX':
        # TODO: the type-definitions generated by `gen-python` don't allow
        #  a simplified dict (i.e. dict[str, str]), we transform the annotations
        #  into a dictionary that contains only the annotation value.
        #  In the future, the python code generator should be fixed to support
        #  dict[str, str], if the object has only two slots of which one is
        #  a key.
        annotations = data.get('annotations')
        if annotations:
            new_annotations = {
                k: {'annotation_value': v}
