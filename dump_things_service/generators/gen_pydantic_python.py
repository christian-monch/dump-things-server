...
class ExtendedPydanticClass(PydanticClass):
    ...
    _environment: ClassVar[Environment] = Environment(
        loader=ChoiceLoader([
            PackageLoader('linkml.generators.pydanticgen', 'templates'),
            PackageLoader('dump_things_service.generators', 'templates'),
        ]),
        trim_blocks=True,
        lstrip_blocks=True,
    )
...
TemplateType = TypeVar("TemplateType", bound=Union[PydanticModule, ExtendedPydanticClass, PydanticAttribute])
    ...
    * :meth:`~.LifecycleMixin.after_render_template`
    ...
    def _template_environment(self) -> Environment:
        ...
        if self.template_dir is not None:
            loader = ChoiceLoader([
                FileSystemLoader(EXTENDED_TEMPLATE_DIR),
    ...
    @overload
    def include_metadata(self, model: ExtendedPydanticClass, source: ClassDefinition) -> ExtendedPydanticClass: ...
