...
class StrawberryGenerator(Generator):
    ...
    def visit_schema(self, **kwargs) -> str:
        ...
        thing_union = f'{thing_union_name} = Union[\n    "Thing",\n    ' + '\n    '.join(
            f'"{cls}",' for cls in thing_subclasses
        ) + '\n]\n\n\n'
        self.item_definitions[thing_union_name] = {
            'lines': [thing_union],
            'dependencies': set(),
        }

        # Create a union of all non-mixin classes
        record_union = f'{record_union_name} = Union[\n    ' + '\n    '.join(
            f'"{name}",' for name, cls in self.schema.classes.items()
            if not cls.mixin
        ) + '\n]\n\n\n'
        self.item_definitions[record_union_name] = {
            'lines': [record_union],
            'dependencies': set(),
        }
